---
title: archivebox list (search)
description: List, filter, and export information about archived snapshots
---

<Note>
  The `list` command has been renamed to `search` in recent versions. Both names work, but `search` is preferred.
</Note>

The `search` (formerly `list`) command queries your archive database, filters snapshots by various criteria, and exports data in multiple formats.
